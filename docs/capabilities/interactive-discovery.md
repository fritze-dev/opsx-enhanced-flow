...
When you run `/opsx:discover`, the system verifies that `openspec/WORKFLOW.md` exists, reads the constitution, the current change directory, and any existing baseline specs. It reads the research Smart Template for artifact instruction and then generates `research.md` with findings about the domain relevant to your change, rating coverage across nine categories.
...
The system verifies that `openspec/WORKFLOW.md` exists before starting discovery. If it is missing, it directs you to run `/opsx:setup` first. It also verifies that a change workspace exists; if no active change is found, it suggests running `/opsx:new` first.
