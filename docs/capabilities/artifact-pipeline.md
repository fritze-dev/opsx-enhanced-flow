...
- **Post-Implementation Commit Before Approval**: After `/opsx:verify` passes, the system commits all implementation changes with the message `WIP: <change-name> — implementation` and pushes to the remote branch before pausing for user approval. This ensures the PR diff is available for review. If push fails, the system continues with a local commit. This WIP commit is distinct from the final commit in the Standard Tasks section, which includes changelog, docs, and version bump.
- **Standard Tasks in Every Task List**: The tasks template includes universal post-implementation steps (changelog, docs, version bump, commit and push) as a final section. Projects can add extras via the constitution's `## Standard Tasks` section, which are appended after the universal steps.
...
After `/opsx:verify` passes during the QA loop, the system commits all implementation changes and pushes to the remote branch. The commit uses the format `WIP: <change-name> — implementation`, consistent with the post-artifact commit pattern. This makes the PR diff available for the user to review before approval. If the push fails, the system creates a local commit and notes that the user should review changes locally. This WIP commit is separate from the final commit in the Standard Tasks section, which includes changelog, docs, and version bump changes.
