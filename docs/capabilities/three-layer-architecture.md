...
- **WORKFLOW.md + Smart Templates Layer**: `openspec/WORKFLOW.md` declares the 7-stage artifact pipeline order, inline action definitions (built-in and custom), apply gate, optional worktree configuration, and project context. Smart Templates in `openspec/templates/` carry per-artifact instructions, output paths, and dependencies in YAML frontmatter. Together they serve as the single source of truth for pipeline structure, action instructions, and artifact generation.
