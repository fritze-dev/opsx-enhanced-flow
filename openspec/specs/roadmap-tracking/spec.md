...
- **THEN** the issue contains a description explaining what the feature would do, why it is valuable, and any known constraints -- sufficient to start a `specshift propose` change for it
...
- **Roadmap items that become full changes**: A roadmap issue may grow in scope and require the full spec-driven workflow (`specshift propose`). The issue SHALL be referenced in the change's research or proposal for traceability, and closed when the change is complete.
