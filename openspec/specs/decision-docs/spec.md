...
Provides Architecture Decision Records (ADRs) generated by `/opsx:docs` from completed changes' design.md Decisions tables. Each architectural decision becomes a formal, searchable record with context, rationale, alternatives, and consequences.

## Requirements

### Requirement: ADR Generation from Change Decisions

The `/opsx:docs` command SHALL generate formal Architecture Decision Records (ADRs) from `## Decisions` tables in completed changes' `design.md` files.
