...
The `/opsx:docs` command SHALL preserve manual ADRs during regeneration. Manual ADRs are files in `docs/decisions/` matching the pattern `adr-M*.md`. They use the `adr-MNNN-slug.md` naming convention (M prefix + 3-digit zero-padded number) to distinguish them from generated ADRs (`adr-NNN-slug.md`). The agent SHALL NOT delete, overwrite, or renumber manual ADR files. Manual ADRs SHALL be included in the `docs/decisions.md` Key Design Decisions table after all generated ADRs, with links like `[ADR-M001](decisions/adr-M001-init-model-invocable.md)`. The agent SHALL extract the Decision and Rationale from the manual ADR's `## Decision` and `## Rationale` sections for the table.
...
#### Scenario: Manual ADR appears in decisions index table
...
- **Manual ADR with missing sections**: If a manual ADR lacks `## Decision` or `## Rationale`, the agent SHALL use the ADR title (from `# ADR-MNNN:` heading) as the decision text and leave the rationale column empty in the decisions index table.
...
- **Transition from old format**: On first run after the docs restructure, `docs/README.md` loses its inline ADR table (moved to `docs/decisions.md`). No manual migration needed.
