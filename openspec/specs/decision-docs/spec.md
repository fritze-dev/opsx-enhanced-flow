...
2. **Change links**: For every `[Change: <name>]` or `[Archive: <name>]` link, verify the change directory exists under `openspec/changes/`. If the change does not exist, the agent SHALL ask the user whether to remove the link or provide the correct change name. No `<!-- REVIEW -->` markers SHALL be left in the final output.
