...
- Product names (Claude Code), commands (`/specshift:*`), and file paths SHALL remain in English
...
1. **Spec links**: For every `[Spec: <name>]` link, verify `docs/specs/<name>.md` exists. If a linked spec does not exist (e.g., it was renamed or split), the agent SHALL attempt to find the successor spec(s) by searching for the capability name in existing specs. If found, replace the broken link with the successor(s). If the successor cannot be determined, the agent SHALL ask the user to identify the correct spec and update the link accordingly. No `<!-- REVIEW -->` markers SHALL be left in the final output.
...
- Change artifacts (design.md, research.md, proposal.md) follow the templates defined in the spec-driven workflow. <!-- ASSUMPTION: Artifacts created by schema-driven workflow -->
