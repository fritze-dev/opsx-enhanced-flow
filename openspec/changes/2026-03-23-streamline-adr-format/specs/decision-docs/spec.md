...
#### Scenario: ADR decision includes inline rationale
- **GIVEN** an archived change with a `design.md` Decisions table containing a "Decision" column value of "Use setup as new name" and a "Rationale" column value of "Descriptive, avoids conflict with built-in /init"
- **WHEN** the agent generates an ADR
- **THEN** the Decision section contains `**Use setup as new name** — Descriptive, avoids conflict with built-in /init` with no separate Rationale section
