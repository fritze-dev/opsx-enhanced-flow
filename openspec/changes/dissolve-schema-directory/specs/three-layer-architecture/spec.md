...
The system SHALL deliver all 12 commands as `skills/*/SKILL.md` files within the Claude Code plugin system. Skills SHALL be categorized as workflow (5: new, ff, apply, verify, archive), governance (5: setup, bootstrap, discover, preflight, sync), or documentation (2: changelog, docs). All skills SHALL be model-invocable (disable-model-invocation: false or absent).
...
#### Scenario: All 12 skills are present
- **GIVEN** a fully installed plugin
- **WHEN** the `skills/` directory is listed
- **THEN** it SHALL contain exactly 12 subdirectories, each with a `SKILL.md` file
