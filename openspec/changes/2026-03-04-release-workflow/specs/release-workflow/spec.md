...
- **gh CLI not available:** Manual release step for GitHub Release is optional — if `gh` is not installed, the maintainer creates the release via the GitHub web UI.
- **Consumer behind multiple versions:** The update process is the same regardless of how many versions behind — marketplace update + plugin update always fetches the latest.

## Assumptions

<!-- ASSUMPTION: plugin.json uses a top-level "version" field with semver string -->
<!-- ASSUMPTION: marketplace.json uses "plugins[0].version" for the version field -->
<!-- ASSUMPTION: Consumers have the gh CLI or access to the GitHub web UI for releases -->
