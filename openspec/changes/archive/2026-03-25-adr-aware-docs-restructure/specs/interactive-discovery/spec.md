...
The system SHALL run an interactive discovery session when the user invokes `/opsx:discover`. Discovery SHALL operate independently from the artifact pipeline -- it generates only the `research.md` artifact and then pauses for user answers. It SHALL NOT generate proposal, specs, design, or any downstream artifacts. The discovery process SHALL: (1) read the constitution for project rules, (2) read the decisions index at `docs/decisions.md` to identify existing architectural decisions, then read specific ADR files from `docs/decisions/` that are thematically relevant to the current change, (3) read the current change directory and existing baseline specs, (4) check whether existing specs reflect the current codebase and note stale-spec risks, (5) generate `research.md` with a coverage assessment rating each category as Clear, Partial, or Missing, and (6) generate targeted clarification questions only for Partial or Missing categories, limited to a maximum of 5 questions prioritized by Impact multiplied by Uncertainty. If all categories are Clear, the system SHALL state that and skip questions. After the user provides answers, the system SHALL record decisions with rationale in the Decisions section of research.md and then stop.

**ADR awareness:** The system SHALL read `docs/decisions.md` as a lightweight index to scan decision titles and rationale. Based on thematic relevance to the current change (matching keywords, affected capabilities, related architectural concerns), the system SHALL identify and read specific ADR files from `docs/decisions/adr-*.md` to extract full Decision and Consequences sections. The system SHALL NOT read all ADR files — only those identified as relevant from the decisions index. If `docs/decisions.md` does not exist, the system SHALL skip ADR awareness and proceed without it. Relevant ADR context SHALL be captured in the "Related Decisions" section of research.md.
...
- **WHEN** the user invokes `/opsx:discover add-caching-layer`
- **THEN** the system reads `docs/decisions.md` and identifies thematically relevant ADRs (e.g., ADRs about architecture patterns)
- **AND** reads only the identified relevant ADR files for full context
...
- **THEN** the system skips ADR awareness
...
- **THEN** the system reads `docs/decisions.md` and finds no thematically relevant ADRs
...
- **THEN** the system reads the constitution, decisions index, relevant ADRs, change directory, and existing baseline specs
...
- **decisions.md exists but is empty or malformed**: The system SHALL skip ADR awareness and proceed without it, same as when the file does not exist.
- **ADR file referenced in decisions.md no longer exists**: The system SHALL skip that ADR and continue with remaining relevant ADRs.
...
- The decisions index at `docs/decisions.md` is generated by `/opsx:docs` and contains a table with Decision, Rationale, and ADR link columns. <!-- ASSUMPTION: decisions.md generated by docs skill -->
